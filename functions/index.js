...
exports.updatePhone = functions.https.onCall((data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Only Authenticated User can Submit Orders Update Phone Number');
  }
  if (Math.ceil(Math.log10(data.phone_number + 1)) != 10) {
    throw new functions.https.HttpsError('invalid-argument', 'Invalid Phone Number was sent')
  }
  else {
    getAuth().setCustomUserClaims(context.auth.uid, { phone_number: data.phone_number })
  }
})
...
exports.newOrder = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Only Authenticated User can Submit Orders');
  } else {
    ...
    const menu = await admin.firestore().collection('menu').doc('menu_items').get()
